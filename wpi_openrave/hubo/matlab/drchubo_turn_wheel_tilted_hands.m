...
if(robottype == 1)
    ...
    orRobotSetDOFValues(robotid,[-0.95 -0.95], [4 22]); %elbows: Left Elbow Pitch: 4; Right Elbow Pitch: 22
end
...
%wheelrot = CTgrasp(1:3,1:3);
%handrot = wheelrot*rodrigues([-pi/2 0 0]);
...
if(robottype == 1)
    ...
    %T0_RH1 = MakeTransform(CTee(1:3,1:3),CTee(1:3,4)-transoffset');
    %T0_LH1 = MakeTransform(CTee(1:3,1:3),CTee(1:3,4)+transoffset');
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Rotate the driving wheel's manipulator's coordinate frame 90 degrees
    % around its ?-axis
    temp = CTee*MakeTransform(rodrigues([0 -pi/2 0]),[0 0 0]');

    % Rotate the new coordinate frame +90 degrees around its ?-axis
    ...
    %T0_RH1 = MakeTransform(rodrigues([0 ((pi/2)+0.4) 0])*handrot,Rhandtrans'-transoffset');
    %T0_LH1 = MakeTransform(rodrigues([0 ((pi/2)+0.4) 0])*handrot,Lhandtrans'+transoffset');
    %T0_RH1 = MakeTransform(handrot*rodrigues([0 0 pi/2]),Rhandtrans'-transoffset');
    %T0_LH1 = MakeTransform(handrot*rodrigues([0 0 pi/2]),Lhandtrans'+transoffset');
elseif(robottype == 2)
...
startik = orProblemSendCommand(['DoGeneralIK exec nummanips 2 maniptm 0 ' num2str([GetRot(T0_LH1),GetTrans(T0_LH1)]) ' maniptm 1 ' num2str([GetRot(T0_RH1),GetTrans(T0_RH1)])],probs.cbirrt);
