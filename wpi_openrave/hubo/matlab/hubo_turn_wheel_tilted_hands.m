...
if(robottype == 1)    
    ...
    orBodySetTransform(crankid,[reshape(rodrigues([0 0 pi/2])*rodrigues([pi/2 0 0]),1,9)';[0.18 0.09 0.9]']) % Note that when mounted on our foldable table the height of the wheel is 0.8128
elseif(robottype == 2)    
...
if(robottype == 1)
    ...
    Left_Hand_Point_In_Wheel_Coordinate_Frame = temp*MakeTransform(rodrigues([0 0 0]),[0 0.15 0]');
    ...
    % Translate the new coordinate frame -0.11 meters on its Z-axis and assign it to Left Hand
    Right_Hand_Point_In_Wheel_Coordinate_Frame = temp*MakeTransform(rodrigues([0 0 0]),[0 -0.15 0]');
