...
def get_pairs(pitch, height, dist, env, robot, myRmaps):
    ...
    T0_PUSH = MakeTransform(matrix(rodrigues([0, pitch, 0])),transpose(matrix([0.0, 0.0, height])))
    TPUSH_LH = MakeTransform(matrix(rodrigues([0, 0, 0])),transpose(matrix([0.0, 0.0, 0.0])))
    T0_LH = dot(T0_PUSH, TPUSH_LH)
    h.append(misc.DrawAxes(env, T0_LH, 0.4))

    TPUSH_RH = MakeTransform(matrix(rodrigues([0, 0, 0])),transpose(matrix([0.0, -dist, 0.0])))
    T0_RH = dot(T0_PUSH, TPUSH_RH)
...
if __name__ == '__main__':
    ...
    delta1 = 0.1
    delta2 = 0.05
    trajs = huboplus_trajectory_generator.get_push(minTrajLength,maxTrajLength,delta1,delta2)
    myLogger = BensLogger(arg_note=str(datetime.now()),arg_name='huboplus_push_samples')
    myLogger.header(['label','pitch','height','traj_length','dist_left_right','left_start_sphere_ind','righ_start_sphere_ind'])
                    ...
                    for t in range(len(trajs[0])):
                        ...
                        if(samples != []):
                            for n in samples:
                                ...
                                # feature4: 
                                myLogger.save([0, pitch, height, minTrajLength+(t*delta1), dist, n[0][0].sIdx, n[1][0].sIdx])
