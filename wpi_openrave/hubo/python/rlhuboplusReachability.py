...
class ReachabilityMap(object):
    def __init__(self, iksolver, robot, manip):
        ...
        for i in range(self.n):
            if(i != 0 and i != (self.n/2)):
                self.rm3D.append(dot(rodrigues([0,0,0]),rodrigues([0,0,i*self.inc1])))
    ...
    def generate(self,env):
        ...
        if(self.name == "rlhuboplus_left"):
            myArmJointIndices = self.robot.GetManipulators()[0].GetArmJoints()
        elif(self.name == "rlhuboplus_right"):
            myArmJointIndices = self.robot.GetManipulators()[1].GetArmJoints()

        self.robot.SetActiveDOFs(myArmJointIndices)

        T0_LH = self.robot.GetManipulators()[0].GetEndEffectorTransform()
        T0_RH = self.robot.GetManipulators()[1].GetEndEffectorTransform()
        T0_LF = self.robot.GetManipulators()[2].GetEndEffectorTransform()
        T0_RF = self.robot.GetManipulators()[3].GetEndEffectorTransform()

        probs_cbirrt = RaveCreateModule(env,'CBiRRT')
         
        try:
            env.AddModule(probs_cbirrt,'rlhuboplus') # this string should match to <Robot name="" > in robot.xml
            ...
            for y in self.yarray:
                for z in self.zarray:
                    ...
                    for rmIdx, rm in enumerate(self.rm3D):
                        ...
                        if(self.name == "rlhuboplus_left"):
                            myIK = probs[0].SendCommand('DoGeneralIK exec supportlinks 2 Body_RAR Body_LAR movecog '+cogtargStr+' nummanips 4 maniptm 0 '+self.trans_to_str(T0_req)+' maniptm 1 '+self.trans_to_str(T0_RH)+' maniptm 2 '+self.trans_to_str(T0_LF)+' maniptm 3 '+self.trans_to_str(T0_RF))
                        elif(self.name == "rlhuboplus_right"):
                            myIK = probs[0].SendCommand('DoGeneralIK exec supportlinks 2 Body_RAR Body_LAR movecog '+cogtargStr+' nummanips 4 maniptm 0 '+self.trans_to_str(T0_LH)+' maniptm 1 '+self.trans_to_str(T0_req)+' maniptm 2 '+self.trans_to_str(T0_LF)+' maniptm 3 '+self.trans_to_str(T0_RF))

                        if(myIK != ''):
                            #print myIK
                            #print str2num(myIK).tolist()
                            #print myArmJointIndices.tolist()
                            self.robot.SetActiveDOFValues(str2num(myIK).tolist())
                            ...
                            close_enough = True
                            if(not allclose(T0_req.round(4),T0_ee.round(4))):
                                #print T0_req
                                #print T0_ee
                            ...
                            if(close_enough):
                                if((not env.CheckCollision(self.robot)) and (not self.robot.CheckSelfCollision())):
                                    ...
                                    s.T.append(Tbase_ee)
                                    s.configs.append(self.robot.GetActiveDOFValues())
